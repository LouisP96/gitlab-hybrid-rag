...
class MetadataProcessor:
    ...
    def process(self, project_name: str) -> List[Document]:
        ...
        self.logger.info(
            f"Processed {len(documents)} metadata documents for {project_name}"
        )
        return documents
    ...
    def _process_issues(self, metadata_dir: Path, project_name: str) -> List[Document]:
        ...
        try:
            ...
            self.logger.info(
                f"Processed {len(documents)} issue documents for {project_name}"
            )
            return documents
    ...
    def _process_merge_requests(
        ...
        try:
            ...
            self.logger.info(
                f"Processed {len(documents)} merge request documents for {project_name}"
            )
            return documents

        except Exception as e:
            self.logger.warning(
                f"Error processing merge requests for {project_name}: {e}"
            )
            return []
    ...
    def _process_issue_discussions(
        self, metadata_dir: Path, project_name: str, issue_iid: int, issue_title: str
    ) -> List[Document]:
        ...
        try:
            ...
            self.logger.warning(
                f"Error processing issue {issue_iid} discussions for {project_name}: {e}"
            )
            return []

    def _process_mr_discussions(
        self, metadata_dir: Path, project_name: str, mr_iid: int, mr_title: str
    ) -> List[Document]:
        ...
        try:
            ...
            self.logger.warning(
                f"Error processing MR {mr_iid} discussions for {project_name}: {e}"
            )
            return []
    ...
    def _process_milestones(
        ...
        try:
            ...
            self.logger.info(
                f"Processed {len(documents)} milestones for {project_name}"
            )
            return documents
