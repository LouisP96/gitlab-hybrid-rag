...
class FileProcessor:
    ...
    def process_code(self, project_name: str) -> List[Document]:
        ...
        return self._process_files(
            code_files, project_name, "code", self._detect_language
        )
    ...
    def process_docs(self, project_name: str) -> List[Document]:
        ...
        return self._process_files(
            doc_files, project_name, "documentation", self._classify_doc_type
        )

    def _process_files(
        self, files: List[Path], project_name: str, file_type: str, classifier_func
    ) -> List[Document]:
        ...
        for file_path in files:
            ...
            try:
                ...
                doc_id = (
                    f"{project_name}_{file_type}_{str(relative_path).replace('/', '_')}"
                )
        ...
        self.logger.info(
            f"Processed {len(documents)} {file_type} files for {project_name}"
        )
        return documents
