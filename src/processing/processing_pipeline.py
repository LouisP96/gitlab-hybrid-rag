...
class ProcessingPipeline:
    ...
    def __init__(
        self, gitlab_backup_dir: Path, output_dir: Path, max_file_size_mb: float = 5.0
    ):
    ...
    def process_project(self, project_name: str) -> Dict[str, List[Document]]:
        ...
        try:
            ...
            self.logger.error(
                f"Error in documentation processor for {project_name}: {e}"
            )
    ...
    def _save_documents(
        self, documents: List[Document], project_name: str, processor_type: str
    ):
