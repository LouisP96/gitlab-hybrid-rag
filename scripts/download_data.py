...
class GitLabDownloader:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(os.path.join(self.config.output_dir, "download_log.txt")),
    ...
    def _get_specific_project(self, project_path: str) -> List[Dict]:
        ...
        if project:
            ...
            self.logger.error("The format should be exactly as shown in GitLab, e.g., 'group/project'")
            return []
    ...
    def download_project_data(self, project: Dict, project_index: int, total_projects: int):
        """Download all data for a single project"""
        project_id = project["id"]
        project_name = project["path_with_namespace"].replace("/", "_")
        
        self.progress.report_project_progress(project_index, total_projects, project['name'])
    ...
    def _clone_repository(self, project: Dict, project_name: str, project_index: int, total_projects: int):
        ...
        if os.path.exists(project_dir):
            self.logger.info(f"Directory {project_dir} already exists, skipping clone...")
            self.clone_stats.skipped += 1
        else:
            self._perform_clone(project, project_dir, project_name)
        
        # Report progress
        self.progress.report_clone_progress(project_index, total_projects, self.clone_stats)
    ...
    def _perform_clone(self, project: Dict, project_dir: str, project_name: str):
        ...
        try:
            ...
            self.logger.info(f"Successfully cloned {project['name']} in {elapsed_time:.1f} seconds")
    ...
    def _download_metadata(self, project: Dict, project_name: str, project_id: int):
        ...
        if self._metadata_exists(metadata_dir):
            self.logger.info(f"Metadata for {project['name']} already exists, skipping...")
            return
    ...
    def _metadata_exists(self, metadata_dir: str) -> bool:
        ...
        return (
            os.path.exists(os.path.join(metadata_dir, "issues.json")) and 
            os.path.exists(os.path.join(metadata_dir, "merge_requests.json"))
        )
    
    def _download_and_save(self, endpoint: str, filename: str, project_id: int, 
                          metadata_dir: str, params: Optional[Dict] = None) -> List[Dict]:
    ...
    def _download_issues(self, project_id: int, metadata_dir: str, project_name: str):
        ...
        issues = self._download_and_save("issues", "issues", project_id, metadata_dir, 
                                       params={"state": "all"})
    ...
    def _download_merge_requests(self, project_id: int, metadata_dir: str, project_name: str):
        """Download merge requests and their discussions"""
        self.logger.info(f"Downloading merge requests for {project_name}...")
        
        merge_requests = self._download_and_save("merge_requests", "merge_requests", 
                                                project_id, metadata_dir, params={"state": "all"})
        self.logger.info(f"Downloaded {len(merge_requests)} merge requests for {project_name}")
    ...
    def _download_wikis(self, project: Dict, project_id: int, metadata_dir: str, project_name: str):
        ...
        try:
            ...
            wiki_pages = wiki_pages_response if isinstance(wiki_pages_response, list) else []
            ...
            self.logger.info(f"Downloaded {successful_downloads}/{len(wiki_pages)} wiki pages for {project_name}")
    ...
    def _download_wiki_page(self, project_id: int, wiki_dir: str, page: Dict) -> bool:
        ...
        try:
            ...
            if page_content:
                ...
                with open(os.path.join(wiki_dir, f"{page_slug}.md"), "w", encoding="utf-8") as f:
                    f.write(content)
                
                return True
            
        except Exception as e:
            self.logger.warning(f"Error downloading wiki page {page.get('slug', 'unknown')}: {str(e)}")
        
        return False
    
    def _download_milestones(self, project_id: int, metadata_dir: str, project_name: str):
        """Download milestones"""
        self.logger.info(f"Downloading milestones for {project_name}...")
        milestones = self._download_and_save("milestones", "milestones", project_id, metadata_dir)
    ...
    def _download_releases(self, project_id: int, metadata_dir: str, project_name: str):
        ...
        releases = self._download_and_save("releases", "releases", project_id, metadata_dir)
    ...
    def _download_ci_data(self, project_id: int, metadata_dir: str, project_name: str):
        ...
        pipelines = self._download_and_save("pipelines", "pipelines", project_id, metadata_dir)
        ...
        for pipeline in pipelines[:self.config.pipeline_limit]:
            ...
            if pipeline_details:
                filepath = os.path.join(metadata_dir, f"pipeline_{pipeline_id}_details.json")
                with open(filepath, "w") as f:
